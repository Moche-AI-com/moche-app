...
export function GapInterview({ jobId }: { jobId: string }) {
  ...
  if (gaps === null) return <button className="btn btn-ghost" type="button" onClick={checkGaps} disabled={loading}>{loading ? 'Checking gaps…' : 'Check remaining setup gaps'}</button>;
  if (!current) return <p className="success">No blocking setup gaps remain. You can continue to your property.</p>;
  return <section className="card" style={{ padding: '1rem' }}><p className="faint" style={{ marginTop: 0 }}>One last question, only because this is still needed for guest readiness.</p><h2 style={{ fontSize: '1.05rem' }}>{current.label}</h2><p>{current.why}</p><textarea className="input" rows={5} value={answer} onChange={(event) => setAnswer(event.target.value)} placeholder="Write the guest-facing detail you want saved…" /><div style={{ display: 'flex', gap: '.75rem', marginTop: '.75rem' }}><button className="btn btn-primary" type="button" disabled={loading || answer.trim().length < 20} onClick={submit}>{loading ? 'Saving…' : 'Save answer'}</button><button className="btn btn-ghost" type="button" onClick={() => setGaps((items) => items ? items.slice(1) : [])}>Skip for now</button></div>{error && <p role="alert" className="alert alert-error">{error}</p>}</section>;
}
