...
export function LocalPlaceForm({ propertyId, place, pickedLocation, onPickLocation }: {
  propertyId: string; place?: LocalPlaceRow; pickedLocation?: PickedLocation | null; onPickLocation?: (target: string) => void;
}) {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => document.getElementById(`${prefix}-lat`)?.focus({ preventScroll: true }));
  ...
  const text = (name: 'name' | 'address' | 'category' | 'website' | 'phone' | 'tags' | 'intentTags', label: string, maxLength: number, type = 'text') => (
    <div style={{ marginTop: '.7rem' }}>
      ...
      <input id={`${prefix}-${name}`} name={name} className="input" value={draft[name]} onChange={(event) => setDraft((d) => ({ ...d, [name]: event.target.value }))} maxLength={maxLength} type={type} required={name === 'name' || name === 'category'} />
    </div>
  );
  ...
  return (
    <form action={action} onReset={(event) => event.preventDefault()} className="ph-no-capture" style={{ borderTop: place ? '1px solid var(--border)' : undefined, marginTop: '.9rem', paddingTop: place ? '.3rem' : 0 }}>
      ...
      {!place && <p className="muted" style={{ fontSize: '.82rem' }}>Add a place you know. Map search suggestions are previews, not automatically saved or endorsed.</p>}
      {text('name', 'Place name', 160)}
      {text('address', 'Address', 500)}
      <p className="faint" style={{ fontSize: '.78rem', margin: '.3rem 0' }}>For reliable directions, add an address or choose a pin on the map.</p>
      {onPickLocation && <button type="button" className="btn btn-sm" style={{ minHeight: 44, marginTop: '.35rem' }} onClick={() => { setDetailsOpen(true); onPickLocation(target); }}><MapPin size={14} aria-hidden /> Choose on map</button>}
      {text('category', 'Category', 80)}
      <div style={{ marginTop: '.7rem' }}>
        <label className="label" htmlFor={`${prefix}-note`}>Host note · shared with guests</label>
        <textarea id={`${prefix}-note`} name="hostNote" className="input" value={draft.hostNote} onChange={(event) => setDraft((d) => ({ ...d, hostNote: event.target.value }))} maxLength={500} rows={3} placeholder="What would you tell a guest about this place?" />
      </div>
      <div style={{ display: 'flex', gap: '.9rem', flexWrap: 'wrap', alignItems: 'center', marginTop: '.75rem' }}>
        <label><input name="isFavorite" type="checkbox" value="true" checked={draft.isFavorite} onChange={(event) => setDraft((d) => ({ ...d, isFavorite: event.target.checked }))} /> Favorite</label>
        <div><label className="label" htmlFor={`${prefix}-status`}>Guest visibility</label><select id={`${prefix}-status`} name="status" className="input" value={draft.status} onChange={(event) => setDraft((d) => ({ ...d, status: event.target.value as typeof d.status }))}>
          <option value="approved">Visible to guests</option><option value="suggested">Only hosts · draft</option><option value="hidden">Hidden · only hosts</option>
        </select></div>
      </div>
      <details open={detailsOpen} onToggle={(event) => setDetailsOpen(event.currentTarget.open)} style={{ marginTop: '1rem' }}>
        <summary style={{ cursor: 'pointer', fontWeight: 600 }}>More details (optional)</summary>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '.7rem', marginTop: '.7rem' }}>
          <div><label className="label" htmlFor={`${prefix}-lat`}>Latitude (optional)</label><input id={`${prefix}-lat`} name="lat" className="input" type="number" step="any" min={-90} max={90} value={lat} onChange={(event) => setLat(event.target.value)} /></div>
          <div><label className="label" htmlFor={`${prefix}-lng`}>Longitude (optional)</label><input id={`${prefix}-lng`} name="lng" className="input" type="number" step="any" min={-180} max={180} value={lng} onChange={(event) => setLng(event.target.value)} /></div>
        </div>
        {(lat || lng) && <button type="button" className="btn btn-sm" style={{ minHeight: 44, marginTop: '.5rem' }} onClick={() => { setLat(''); setLng(''); }}>Clear coordinates</button>}
