...
export default async function AppliancesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: appliances }, { data: sections }] = await Promise.all([
    client.from('property_appliances').select('*').eq('property_id', (await params).id).order('created_at', { ascending: false }),
    client.from('appliance_manual_sections').select('*').eq('property_id', (await params).id).order('created_at', { ascending: false }),
  ]);
