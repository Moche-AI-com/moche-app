...
type Filter = 'all' | 'unread' | 'escalations';
...
export function GuestChatInbox({ propertyId, stayId, canAnnounce }: { propertyId: string; stayId: string | null; canAnnounce: boolean }) {
  ...
  const visibleThreads = useMemo(
    () => threads.filter((thread) => (filter === 'unread' ? thread.unreadCount > 0 : filter === 'escalations' ? thread.pinned : true)),
    [threads, filter],
  );
        ...
        <div role="list" aria-label="Guest conversations" className="chat-thread-list">
          {loadingThreads ? (
            ...
            <p className="muted" style={{ padding: '.5rem' }}>
              {filter === 'all' ? 'No guest chats yet for this stay.' : filter === 'unread' ? 'Nothing unread.' : 'No open escalations.'}
            </p>
          ) : (
