...
export default async function ServiceRequestsPage({
  ...
  if (queryPropIds.length) {
    const [{ data: tickets }, { data: contactRows }, activeRes, pastRes] = await Promise.all([
      ...
      // types` run; until then this one query opts out of column typechecking.
      (supabase as any)
        .from('service_requests')
        .select(
          'id, property_id, description, service_type, status, urgency, resolution_notes, created_at, archived_at, location_note, likely_causes, suggested_parts, access_instructions, guest_availability, summary, media_urls, interview_status, assigned_contact_id, edited_summary, edited_details',
        )
