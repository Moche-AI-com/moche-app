...
// Workflow 4 — Extras & Amenities. Browse property-specific offers, pick
// variants/quantity, and submit a request. Empty state when the property has
// no active extras.
export function ExtrasWorkflow(props: {
  ...
  const loadOrders = useCallback(async () => {
    try {
      ...
      const list: OrderRow[] = Array.isArray(json) ? json : (json.orders ?? []);
      setOrders(list);
  ...
  useEffect(() => {
    if (view === 'requests') void loadOrders();
  }, [view, loadOrders]);
  ...
  async function submit() {
    ...
    if (selected.options && selected.options.length > 0 && !variant && selected.kind !== 'package') {
      setError('Please choose an option before requesting this.');
      return;
    }
    setBusy(true);
    setError(null);
    try {
      const res = await fetch(`/api/guest/${props.slug}/extras-request`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          offerId: selected.id,
          quantity: selected.kind === 'package' ? undefined : quantity,
      ...
      setRequestNumber(typeof json.requestNumber === 'string' ? json.requestNumber : null);
      setView('done');
    } catch (err) {
  ...
  const groups = CATEGORY_ORDER
    .map((cat) => ({
      cat,
      items: props.offers.filter((o) => (o.category ?? 'more') === cat),
    }))
    ...
    <section aria-label="Extras and amenities" style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
      <div className="gp-wf-header">
        <button type="button" className="gp-back" onClick={view === 'browse' ? props.onBack : () => setView('browse')}>
          <ArrowLeft size={16} aria-hidden /> {view === 'browse' ? 'Menu' : 'Back'}
        </button>
        <span className="gp-wf-title">Extras & Amenities</span>
      </div>
        ...
        <>
          {props.offers.length === 0 ? (
            <div className="gp-empty">
              ...
              <div>No extras or amenities are available for this stay right now.</div>
              <div style={{ marginTop: 16 }}>
                <button type="button" className="gp-btn gp-btn-ghost" onClick={props.onBack}>Back to menu</button>
              </div>
            </div>
          ) : (
            <>
              {groups.map((g) => (
                <div key={g.cat}>
                  <div className="gp-cat">{CATEGORY_LABEL[g.cat] ?? g.cat}</div>
                  {g.items.map((o) => (
              ...
              <div style={{ marginTop: 14 }}>
                <button type="button" className="gp-btn gp-btn-ghost" onClick={() => setView('requests')}>
                  Your requests
                </button>
              </div>
            </>
      ...
      {view === 'detail' && selected && (
        <div className="gp-card">
          ...
          {selected.details || selected.description ? (
            <p className="gp-step-sub" style={{ marginTop: 10 }}>{selected.details ?? selected.description}</p>
          ) : null}

          {selected.options && selected.options.length > 0 ? (
            <div className="gp-field">
              <span className="gp-label">{selected.option_label ?? 'Options'}</span>
              <div className="gp-variant-row">
                {selected.options.map((opt) => (
                  <button
                    key={opt}
                    type="button"
                    className={`gp-variant ${variant === opt ? 'gp-variant-on' : ''}`}
          ...
          {selected.kind !== 'package' ? (
            <div className="gp-field">
              <span className="gp-label">Quantity{selected.unit_label ? ` (${selected.unit_label})` : ''}</span>
              <div className="gp-stepper">
                <button type="button" onClick={() => setQuantity((q) => Math.max(1, q - 1))} aria-label="Decrease quantity">
                  <Minus size={16} aria-hidden />
                </button>
                <span>{quantity}</span>
                <button type="button" onClick={() => setQuantity((q) => Math.min(maxQty, q + 1))} aria-label="Increase quantity">
          ...
          <div className="gp-field">
            <label className="gp-label" htmlFor="gp-extra-when">Preferred time (optional)</label>
            <input
              id="gp-extra-when"
              className="gp-input"
              type="datetime-local"
              value={preferredFor}
              onChange={(e) => setPreferredFor(e.target.value)}
            />
          </div>

          <div className="gp-field">
            <label className="gp-label" htmlFor="gp-extra-note">Note for the host (optional)</label>
            <textarea
              id="gp-extra-note"
          ...
          <button type="button" className="gp-btn gp-btn-primary" onClick={() => void submit()} disabled={busy}>
            {busy ? 'Sending…' : (selected.cta_label ?? 'Request')}
          </button>
        </div>
      )}
      ...
      {view === 'done' && (
        <div className="gp-card gp-confirm">
          ...
          <h2 className="gp-step-title" style={{ marginTop: 0 }}>Request sent</h2>
          {requestNumber ? (
            <>
              <div style={{ fontSize: '0.85rem', opacity: 0.65 }}>Reference</div>
              <div className="gp-ref">{requestNumber}</div>
            </>
          ) : null}
          <p className="gp-step-sub" style={{ marginBottom: 18 }}>
            Your host has been notified and will confirm shortly.
          </p>
          <button type="button" className="gp-btn gp-btn-primary" onClick={() => setView('browse')}>Browse more</button>
          <div style={{ height: 10 }} />
          <button type="button" className="gp-btn gp-btn-ghost" onClick={props.onBack}>Back to menu</button>
        </div>
      )}
      ...
      {view === 'requests' && (
        <>
          {!ordersLoaded ? (
            <div className="gp-empty"><Loader2 size={18} className="gp-spin" aria-label="Loading requests" /></div>
          ) : orders.length === 0 ? (
            <div className="gp-empty">You haven&apos;t requested anything yet.</div>
          ) : (
            orders.map((o) => (
