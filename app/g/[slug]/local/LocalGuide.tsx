...
import { LiveNearbySearch } from './LiveNearbySearch';
...
function categoryLabel(category: string): string { return NEARBY_CATEGORY_LABEL[category] ?? category.replace(/_/g, ' '); }
function categoryPlural(category: string): string { return NEARBY_CATEGORY_LABEL_PLURAL[category] ?? `${categoryLabel(category)}s`; }
  ...
  return (
    <article className="gp-place-card">
      ...
      <div className="gp-place-body">
        ...
        <div className="gp-place-meta"><span>{categoryLabel(place.category)}</span>{distance ? <span>· {distance}</span> : null}
          {typeof place.rating === 'number' ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}>· <Star size={12} aria-hidden /> {place.rating.toFixed(1)}</span> : null}</div>
...
export function LocalGuide(props: {
  ...
  places: GuestLocalPlace[]; loadError?: boolean; liveNearbyEnabled: boolean;
}) {
  const { theme, toggleTheme } = usePortalTheme();
  const [query, setQuery] = useState('');
  const [category, setCategory] = useState('all');
  const [showAll, setShowAll] = useState(false);
  const [mode, setMode] = useState<'guide' | 'nearby'>('guide');
    ...
    <div className={`gp-v2 ${theme === 'light' ? 'gp-light' : ''} ${props.fontClassName}`} style={brandVars}>
      ...
      <div className="gp-wrap">
        ...
        <main className="gp-main">
          ...
          {props.liveNearbyEnabled && <div className="gp-filter-bar" role="group" aria-label="Local discovery mode">
            <button type="button" className={`gp-filter-chip ${mode === 'guide' ? 'gp-filter-chip-on' : ''}`} aria-pressed={mode === 'guide'} onClick={() => setMode('guide')}>Host guide</button>
            <button type="button" className={`gp-filter-chip ${mode === 'nearby' ? 'gp-filter-chip-on' : ''}`} aria-pressed={mode === 'nearby'} onClick={() => setMode('nearby')}>Explore nearby</button>
          </div>}
          {props.liveNearbyEnabled && mode === 'nearby' ? <LiveNearbySearch slug={props.slug} /> : props.loadError ? (
            <div role="alert" className="gp-empty">Local recommendations could not be loaded. Please try again in a moment or ask your host.</div>
