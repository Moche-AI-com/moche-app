...
const TERMINAL_ACTIONS = [
  'guest.review_nudge.response',
  'guest.review_nudge.dismissed',
  'guest.review_nudge.clicked',
];

const feedbackSchema = z.object({
  action: z.enum(['impression', 'response', 'dismiss', 'click']),
...
async function relatedSessionIds(admin: ReturnType<typeof createAdminClient>, session: Awaited<ReturnType<typeof getGuestSession>>) {
  if (!session) return [];
  ...
  const { data: rows } = await (admin as any)
    ...
    .eq('guest_identity_id', current.guest_identity_id);
...
export async function GET() {
  ...
  const { data: settings } = await admin
    ...
    .select('review_nudge_enabled, review_nudge_auto, review_url, confidence_threshold')
    .eq('property_id', session.propertyId)
    .maybeSingle();
  const reviewUrl = safeReviewUrl(settings?.review_url);
  if (settings?.review_nudge_enabled !== true || !reviewUrl) {
  ...
  if (priorFeedback?.length || priorEvent?.length) {
    return NextResponse.json({ eligible: false });
  }

  const automatic = settings.review_nudge_auto === true;
  if (!automatic) {
    return NextResponse.json({ eligible: true, automatic: false, shouldPrompt: false, reviewUrl });
  }
  ...
  if (conversationIds.length > 0) {
    const { data } = await (admin as any)
      ...
      .gte('confidence', Number(settings.confidence_threshold ?? 0.55))
      .limit(1);
    helpfulAnswer = Boolean(data?.length);
  }
  ...
  return NextResponse.json({
    eligible: true,
    automatic: true,
    shouldPrompt: hasSuccessfulMoment && !hasActiveProblem,
    reviewUrl,
  });
}
...
export async function POST(req: Request) {
  ...
  const admin = createAdminClient();
  const rate = await checkRateLimit(admin, { key: `review_nudge:${session.sessionId}`, action: 'guest.review_nudge', limit: 12, windowSeconds: 60 * 60 });
  ...
  const { data: settings } = await admin
    ...
    .eq('property_id', session.propertyId)
    .maybeSingle();
  if (!property || settings?.review_nudge_enabled !== true || !safeReviewUrl(settings.review_url)) {
  ...
  if (action === 'response') {
    ...
    const write = existing
      ...
      : admin.from('product_feedback').insert({ source: 'guest', rating: ratingForMood(mood), comment: detail, property_id: session.propertyId, guest_session_id: session.sessionId, page: 'guest_portal_review_nudge' } as never);
  ...
  if (action !== 'impression') {
    await admin.from('audit_logs').insert({
      ...
      metadata: { stay_id: session.stayId, mood: mood ?? null, category: category ?? null },
    } as never);
  }

  try {
    await capture(eventName, session.sessionId, { property_id: session.propertyId, stay_id: session.stayId, mood: mood ?? null, category: category ?? null });
  } catch {
    // Analytics must never block the guest flow.
  }
