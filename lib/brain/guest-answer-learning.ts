...
/**
 * Dedicated high-reliability model path for turning a host reply + attached thread
 * into a proposed guest-safe Q/A. This intentionally does not use the lightweight
 * concierge default. Set AI_BRAIN_LEARNING_MODEL to override the production model.
 */
export async function normalizeGuestAnswerForBrain(input: GuestAnswerLearningInput): Promise<NormalizedGuestAnswer> {
  ...
  const model = process.env.AI_BRAIN_LEARNING_MODEL ?? 'openai/gpt-4o';
  ...
  if (!response.ok) {
    log.warn('guest_answer_learning_failed', { status: response.status, model });
    throw new Error('Could not normalize the guest answer.');
  }
