...
function row(id: string, role: string, content: string, safe = false): GuestHistoryRow {
  return { id, role, content, created_at: '2026-09-25T19:00:00Z', model: role === 'assistant' ? 'openai/gpt-4o' : null, guest_replay_safe: safe };
}

describe('guest AI history replay', () => {
  it('returns stable database IDs and preserves newly vetted replies after Wi-Fi turns', () => {
  ...
  it('masks a bare legacy answer and all later unverified implicit follow-ups', () => {
    const result = serializeGuestHistory([row('q1', 'guest', 'Wi-Fi password?'), row('a1', 'assistant', 'synthetic secret'), row('q2', 'guest', 'Repeat that'), row('a2', 'assistant', 'blue turtles jump')]);
    expect(result[1].content).toBe(LEGACY_WIFI_NOTICE);
    expect(result[3].content).toBe(LEGACY_WIFI_NOTICE);
  });
  it('masks a legacy Wi-Fi reply in a delta after the question', () => {
    expect(serializeGuestHistory([row('a1', 'assistant', 'Old unverified answer')], true)[0].content).toBe(LEGACY_WIFI_NOTICE);
  });
