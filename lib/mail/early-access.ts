...
// Branded pre-launch thank-you shell. Self-contained (dark-on-brand card, teal gradient
// button) so it does not depend on the auth-email module's private helpers.
function render(firstName: string): { html: string; text: string } {
  const heading = firstName ? `Thanks, ${escapeHtml(firstName)} — you're on the list` : "Thanks — you're on the list";
  const url = `${publicEnv.appUrl}/`;
  ...
  <tr><td align="center">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#0E1826;border:1px solid rgba(157,176,198,0.14);border-radius:16px;overflow:hidden;">
      <tr><td style="padding:32px 32px 8px 32px;">
        <span style="font-size:20px;font-weight:600;letter-spacing:-0.02em;color:#EAF1FA;">Moche<span style="color:#33E6D4;">-AI</span></span>
      </td></tr>
      <tr><td style="padding:16px 32px 0 32px;">
        <h1 style="margin:0 0 12px 0;font-size:22px;line-height:1.3;color:#EAF1FA;">${heading}</h1>
        <p style="margin:0 0 24px 0;font-size:15px;line-height:1.6;color:#9DB0C6;">Your spot is confirmed. We are putting the finishing touches on Moche-AI and we go live on <strong style="color:#EAF1FA;">January&nbsp;1,&nbsp;2027</strong> — we will email you the moment your workspace is ready.</p>
      </td></tr>
      <tr><td align="center" style="padding:0 32px 8px 32px;">
        <a href="${url}" style="display:inline-block;background:#33E6D4;background-image:linear-gradient(115deg,#33E6D4 0%,#58C7E0 45%,#7C8CFF 100%);color:#04121A;font-size:15px;font-weight:700;text-decoration:none;padding:14px 28px;border-radius:10px;">See what is coming</a>
      </td></tr>
      <tr><td style="padding:16px 32px 0 32px;">
      ...
      <tr><td style="padding:8px 32px 28px 32px;border-top:1px solid rgba(157,176,198,0.12);">
        <p style="margin:16px 0 0 0;font-size:12px;line-height:1.6;color:#5F7793;">Moche-AI · Built in Somerville, MA</p>
      </td></tr>
    </table>
  </td></tr>
</table>
...
// Sends the pre-launch early-access thank-you after someone joins the list from the
// /welcome holding page or the landing form. Returns false (and logs) rather than
// throwing so a delivery failure never loses the signup.
export async function sendEarlyAccessThanks(params: {
  ...
  try {
    ...
    const { html, text } = render(firstName);
