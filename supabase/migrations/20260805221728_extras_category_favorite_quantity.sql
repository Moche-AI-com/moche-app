...
--
-- Decision: keep the shipped per-property guest_extras table and extend it,
-- rather than splitting it into an account-wide extras_catalog + property_extras
-- pair. guest_extras is live, guest-facing, and already has its own RLS; a
-- two-table migration would put a working feature at risk for benefits that are
-- reachable additively. See docs/decisions/extras-catalog-shape.md.
--
-- Additive only: no existing row changes behaviour. category defaults to NULL,
-- which the app normalizes to the 'more' bucket, so every existing extra keeps
-- showing without host action.
