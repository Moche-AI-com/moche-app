...
CREATE POLICY notif_select ON public.notifications AS PERMISSIVE FOR SELECT TO authenticated USING (((is_account_member(host_account_id) OR is_account_owner(host_account_id)) AND ((property_id IS NULL) OR can_access_property(property_id)) AND ((recipient_profile_id IS NULL) OR (recipient_profile_id = (SELECT auth.uid())))));
CREATE POLICY notif_update ON public.notifications AS PERMISSIVE FOR UPDATE TO authenticated USING (((is_account_member(host_account_id) OR is_account_owner(host_account_id)) AND ((property_id IS NULL) OR can_access_property(property_id)) AND ((recipient_profile_id IS NULL) OR (recipient_profile_id = (SELECT auth.uid()))))) WITH CHECK (((is_account_member(host_account_id) OR is_account_owner(host_account_id)) AND ((property_id IS NULL) OR can_access_property(property_id)) AND ((recipient_profile_id IS NULL) OR (recipient_profile_id = (SELECT auth.uid())))));
